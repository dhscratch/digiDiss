jQuery(document).ready(function($) {
  ...
  if ((q) && (q != '')){
    ...
    $("#digi-selection .col-md-10").append("<a class='themebutton btn btn-more' href='#' data-type='q' data-val='"+params.q+"'>"+params.q+" <span class='fa fa-close'></span></a>");
  }
  if (template == 'collections'){
    params.f['type_sim'] = 'Collection';
    $("#digi-facets").hide();
    $("#digi-docs").css("width", "100%");
  }
  ...
  function get_data(params){
    ...
    $("#digi-loading").html("<h2>Loading...<br/><span class='fa fa-spinner fa-spin'></span></h2>").show();
    $.ajax({
        type: 'POST',
    ...
    success: function(data)
     {
      $("#digi-loading").hide();
        ...
        if (data == null) {
          $("#digi-content").html(errors.search.fail_null);
        } else if (data == -1){
          $("#digi-content").html(errors.search.fail_null);
        } else if (data.error) {
          $("#digi-content").html(errors.search.no_results);
          if (template == 'collections'){
            $("#digi-content").html(errors.search.no_sub_collections);
          }
          ...
          if (params.show_facets == true){
            ...
            $("#digi-facets").hide();
            $("#digi-docs").addClass("col-md-12").removeClass("col-md-9");
          }
          resultize(data.response.response);//send to resultize function
          clickable(data);
          $("#digi-sort").show();
          if ($("#digi-selection").find("a").length < 1){
            $("#digi-selection").hide();
          }
        } else {
          $("#digi-content").html(errors.search.no_results);
    ...
    {
      $("#digi-content").html("<div class='alert error alert-error'>"+errors.search.fail_null+"</div>");
    }
  });
  }
  ...
  function paginate(data){
    $("#digi-item-count").html("<h6>Displaying " + data.start + " to " + data.end + " of " + data.total_count + "</h6>");
    $("#digi-per-page-div").html("<h6>Show <select id='digi-per-page'><option val='10'>10</option><option val='20'>20</option><option val='50'>50</option></select> per page</h6>");
    $("#digi-per-page").val(params.per_page);
    $("#digi-sort-option").val(params.sort);
    if (data.num_pages > 1) {
      ...
      pagination += "</li>";
      $("#digi-pagination ul.pag").html(pagination);
      $("#digi-pagination").show();
    } else {
      $("#digi-pagination ul.pag").html("");
      $("#digi-pagination").hide();
    }
  }//end paginate
  ...
  function facetize(data){
    ...
    $(".digi-facet-toggle").remove();
    $("#digi-facets").before("<button class='themebutton button btn visible-phone hidden-tablet hidden-desktop digi-facet-toggle hidden-md hidden-lg visible-sm visible-xs'>Show Facets</button>");
  }//end facetize
  ...
  function resultize(data){
    ...
    $.each(data.docs, function(doc, doc_vals){
      ...
      if (template == 'search'){
        //search = grid
        this_doc += "<div class='digi-item search panel panel-default'><div class='panel-body'><div class='one_fourth col-sm-3'><figure><a href='"+this_doc_url+"'>";
        if (thumbnail[1]) {
        ...
        if (search_options.indexOf('Title') > -1){
          this_doc += "<h4 class='digi-item-title'><a href='"+this_doc_url+"'>" + title + "</a></h4>";
        }
        if (creator && search_options.indexOf('Creator') > -1){
          this_doc += "<h6 class='digi-item-creator'>"+ creator + "</h6>";
        }
        if (abstract  && search_options.indexOf('Abstract') > -1){
          this_doc += "<p class='digi-item-abstract'>" + abstract + "</p>";
        }
        if (date  && search_options.indexOf('Date') > -1){
          this_doc += "<p class='digi-item-date'>" + date + "</p>";
        }
        this_doc += "<div class=''><a href='"+this_doc_url+"' class='themebutton button btn'>View More</a></div></div></div></div>";
      } else {
        //browse = tile
        this_doc += "<div class='digi-item browse one_third ";
        if (template == 'collections'){
        ...
        if (browse_options.indexOf('Title') > -1){
          this_doc += "<h5 class='digi-item-title'><a href='"+this_doc_url+"'>"+title+"</a></h5>";
        }
    ...
    $("#digi-docs").html(docs_html);
  }//end resultize

  function clickable(data){
    $("#digi-pagination a").on("click", function(e){
      e.preventDefault();
    ...
    $("#digi-per-page").on("change", function(){
    ...
    $("#digi-facets a").on("click", function(e){
      e.preventDefault();
      console.log("clicked");
      ...
      params.page = 1;
      $("#digi-selection").show();
      $("#digi-selection .col-md-10").append("<a class='themebutton btn btn-more' href='#' data-type='f' data-facet='"+facet+"' data-val='"+facet_val+"'>"+titleize(facet)+" > "+facet_val+" <span class='fa fa-close'></span></a>");
      get_data(params);
    });

    $("#digi-selection a").on("click", function(e){
      e.preventDefault();
  ...
  $("#digi-sort").html("<h6>Sort By: <select id='digi-sort-option'><option value='score+desc%2C+system_create_dtsi+desc'>Relevance</option><option value='title_ssi%20asc'>Title A-Z</option><option value='title_ssi%20desc'>Title Z-A</option><option value='creator_tesim%20asc'>Creator A-Z</option><option value='creator_tesim%20desc'>Creator Z-A</option><option value='system_modified_dtsi%20asc'>Date (earliest to latest)</option><option value='system_modified_dtsi%20desc'>Date (latest to earliest)</option></select></h6>");

  $("#digi-sort-option").on("change", function() {
    params.sort = $(this).val();
    get_data(params);
    $(this).val(params.sort);
  });

  $("#digi-content").on("click", ".digi-facet-toggle",  function() {
    $("#digi-facets").toggleClass("hidden-xs hidden-sm visible-sm visible-xs");
    $(".digi-facet-toggle").html($('.digi-facet-toggle').text() == 'Hide Facets' ? 'Show Facets' : 'Hide Facets');
  });
  ...
  function get_wp_data(query, page){
    if (template == 'search'){
      ...
      $.ajax({
          ...
          $("#secondary").addClass('digi-sidebar');
          $("#primary").addClass('digi-main');
